...
auto solve() {
    int n;cin>>n;
    // if prime number than 1
    if(isPrime(n)) return 1;
    // if even than 2, as all even are sum of 2 primes
    if(n%2==0) return 2;
    // if odd
    // check if n-2 is prime than 2
    if(isPrime(n-2)) return 2;
    // else 3 is answer as n-3 is even it will have answer 2 and 3 is another prime thus 3 overall
    return 3;
}
