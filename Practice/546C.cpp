...
auto solve() {
    ...
    int k1;cin>>k1;
    queue<int> q1,q2;
    while(k1--) cin>>e, q1.push(e);
    int k2;cin>>k2;
    while(k2--) cin>>e, q2.push(e);
    int war = 0, cnt = 0;
    while (!q1.empty() and !q2.empty()) {
        war++, cnt++;
        int f1 = q1.front(), f2 = q2.front();
        q1.pop(), q2.pop();
        string key = to_string(f1)+" "+to_string(f2);
        if(f1>f2) {
            q1.push(f2);
            q1.push(f1);
        } else {
    ...
    if(q1.empty()) cout<<war<<" "<<2<<endl;
    else cout<<war<<" "<<1<<endl;
    return;
}
