...
auto solve() {
    ...
    vi pos_a, pos_b;
    int ans = 0;
    pos_a.pb(-1); pos_b.pb(-1);
    for(int i = 0; i < n; i++) if(s[i] == 'a') pos_a.pb(i); else pos_b.pb(i);
    pos_a.pb(n); pos_b.pb(n);
    int i = 1, j = k;
    if(len(pos_a) - 2 <= k or len(pos_b) - 2 <= k){
        return n;
    }
    while(j < pos_a.size() - 1){
        ans = max(ans, pos_a[j + 1] - pos_a[i - 1] - 1);
        i++; j++;
    }
    i = 1, j = k;
    while(j < pos_b.size() - 1){
        ans = max(ans, pos_b[j + 1] - pos_b[i - 1] - 1);
        i++; j++;
    }
    return ans;
}
